...
<%
    ...
    Else
        ...
        Function updateEmployee()
            ...
            cmdPrep.CommandText = "SET NOCOUNT ON; UPDATE [User] SET nameUser=?, birthday=?, phone=?, address=?, email=?, avatar=? WHERE idUser=(select idUser from Employee where idEmployee = ? )"
            ...
            'Session("Success") = "old file name: " &oldFileName
            cmdPrep.execute
            Set cmdPrep = Server.CreateObject("ADODB.Command")
            cmdPrep.ActiveConnection = connDB
            cmdPrep.CommandType = 1
            ...
            if not result.EOF then
                ...
                Else
                    ...
                    If (result("phone") = phoneTemp) Then
                        ...
                        if (isnull(newFileName) or trim(newFileName) = "") then
                ...
                if (cint(idEmployee) = 0) then
                    ...
                    cmdPrep.CommandType = 1
                    cmdPrep.Prepared = True
                    cmdPrep.CommandText = "SET NOCOUNT ON; INSERT INTO [User](nameUser, birthday, phone, address, email, avatar) VALUES (?,?,?,?,?,?); SELECT SCOPE_IDENTITY() as ID"
                    cmdPrep.parameters.Append cmdPrep.createParameter("nameUser",202,1,255,nameUser)
                    cmdPrep.parameters.Append cmdPrep.createParameter("birthday",202,1,255,birthday)
                    cmdPrep.parameters.Append cmdPrep.createParameter("phone",202,1,255,phone)
                    cmdPrep.parameters.Append cmdPrep.createParameter("address",202,1,255,address)
                    cmdPrep.parameters.Append cmdPrep.createParameter("email",202,1,255,email)
                    cmdPrep.parameters.Append cmdPrep.createParameter("avatar",202,1,255,newFileName)
                    set result = cmdPrep.execute
