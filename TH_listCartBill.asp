...
' trang hien tai
    ...
    if (Session("role") = "CUSTOMER") then
        cmdPrep.CommandText = "SELECT COUNT(idCart) AS count FROM [Bill] where dateBill = CONVERT(date, ?, 103) and idCart = (select idCart from Cart where idUser = "&Session("idUser")&")"
    else
                                        ...
                                        do while not result.EOF              
                                            ...
                                            <tr>
                                                ...
                                                <td style="min-width: 120px; width: 11%;  text-align: center;">
                                                    <a href="L_BillUser.asp?idBill=<%=result("idBill")%>" class="btn btn-outline-success" style="padding: 5px 10px;">
